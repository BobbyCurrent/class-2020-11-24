...
```{r sc1}
week_11_small <- rep_sample_n(week_11, 10000)
...
new_thing <- tibble(treatment = c("Control", "Neighbors"),
       solo = TRUE,
       primary_04 = "Yes")

predict <- posterior_predict(fit_1, newdata = new_thing) %>%
  as_tibble() %>%
  rename(Control = `1`,
         Neighbors = `2`) %>%
  mutate(causal_effect = (Control - Neighbors)) %>%
  ggplot(aes(x = causal_effect)) +
  geom_histogram(aes(y = after_stat(count/sum(count))))

predict

posterior_epred(fit_1, newdata = new_thing) %>%
  as_tibble() %>%
  rename(Control = `1`,
         Neighbors = `2`) %>%
  mutate(causal_effect = (Control - Neighbors)) %>%
